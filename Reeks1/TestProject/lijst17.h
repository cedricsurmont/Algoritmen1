...
// te doen....
	...
	Lijst() : std::unique_ptr<Lijstknoop<T>>() {};			//default constructor
	Lijst(const Lijst<T> &lijst);					//copy constructor
		//overloop lijst
	...
	Lijst(Lijst<T> &&lijst);
	//Lijst(Lijstknoop<T> &&lijstknoop);					//move constructor
	Lijst(Lijstknoopptr<T> &&lijstknoopptr);				//move constructor with uniqueptr
	~Lijst() {};											//destructor

	Lijst & operator=(const Lijst& lijst);					//copy assignment operator with Lijst
	Lijst & operator=(Lijst && lijst);						//move assignment operator with Lijst
	Lijst & operator=(Lijstknoopptr<T> && lijstknoopptr);	//move assignment operator with uniqueptr
	
	//move operator calls the move assignment operator
...
template<class T>
inline Lijst<T>::Lijst(Lijst<T>&& lijst) : std::unique_ptr<Lijstknoop<T>>(std::move(lijst)) {};


template<class T>
inline Lijst<T>::Lijst(Lijstknoopptr<T>&& lijstknoopptr)
{
	cout << "move constructor with lijstknoopptr is used" << endl;
}
...
Lijst<T>& Lijst<T>::operator=(const Lijst<T>& lijst)
{
	if (&lijst != this) {
		...
		while (hulpknoop != nullptr) {
			hulp->voegToe(hulpknoop->sleutel);
...
template<class T>
inline Lijst<T>& Lijst<T>::operator=(Lijst<T>&& lijst)
{
	cout << "move assignment operator with Lijst is used" << endl;
	Lijstknoopptr<T>::operator=(std::move(lijst));
	return *this;
}
template<class T>
inline Lijst<T>& Lijst<T>::operator=(Lijstknoopptr<T>&& lijstknoopptr)
{
...
void Lijst<T>::voegToe(const T& sleutel) {
	...
	//'this' is een volledig lijst object, wijst naar het begin van de lijst
	//'nieuw' is een nieuwe lijstknoop, en zijn attribuut 'volgend'(= lijstobject) wijst nog naar niets
	//je swapt de nieuw->volgend (een lijstobject) met this (een gevuld lijstobject)
	//DUS, resultaat: nieuw is de nieuwe knoop, gevolgd door de volledige lijst die al bestond (vooraan toevoegen)
	//en this wijst naar wat nieuw-volgend wees, aka niets
	//daarom moet je nog de nieuwe lijst moven naar *this;
